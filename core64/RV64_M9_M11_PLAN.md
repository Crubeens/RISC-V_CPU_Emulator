...
- 修复 Windows libslirp 注册轮询套接字时调用空回调导致的空地址写入崩溃；
  回调、队列峰值、套接字生命周期和轮询错误均纳入诊断统计。
- Windows 网络轮询改用按 `FD_SETSIZE` 分批的 `select()`，避免 `WSAPoll`
  在 HTTPS 多连接阶段持续失败；非 Windows 平台继续使用 `poll()`。
- 增加可选真实 DNS 测试，覆盖 DHCP/DNS 地址、ARP 请求应答和公网 DNS 回包；
  默认测试不依赖公网。
- 增加独立验收盘生成脚本和两阶段客户机脚本。APT 禁用语言索引，优先 gzip，
  关闭流水线与磁盘包缓存，并移除重复 Debian 软件源，减少解释执行开销。
- 直接作为 PID 1 运行的验收脚本在强制关机前同步文件系统；普通 Debian 镜像
  仍使用正常 systemd 启停流程。测试盘经 ext4 日志回放后离线检查完全一致。

验收记录：

- 第一阶段经 Debian 官方 HTTPS 源完成 `apt update`，安装并运行 riscv64
  `hello`，执行仅升级检查，并确定拒绝损坏的 deb 包。
- 关机后使用同一 ext4 文件再次启动，确认 `hello` 和 dpkg 状态保持，随后卸载
  并再次正常关机；两个阶段完成标记均可从磁盘离线读取。
- 第一阶段 26,967,178,780 machine steps、2819.796 秒、9.564 Msteps/s；第二
  阶段 4,997,341,441 steps、529.659 秒、9.435 Msteps/s。耗时主要来自客户机
  索引解压和包数据库处理，不是 SDL 刷新或网络丢包。
- 第一阶段 VirtIO Net TX/RX 分别为 1349/9533，待处理峰值 18；两阶段网络
  丢包和 libslirp 轮询错误均为 0。

### M11.4：确定失败路径

状态：已完成（2026-08-01）。

- 使用隔离的 APT source、lists 和 cache 目录验证拒绝连接代理；启用
  `APT::Update::Error-Mode=any`，避免 APT 把全量下载失败仅作为成功警告。
- 使用不存在的 Debian suite 验证 404/缺少 Release 文件，而不是把 DNS 或
  其他临时网络错误误判为无效源测试通过。
- 对 Debian 官方 HTTPS 站点设置故意错误的公钥 pin，要求 curl 返回专用的
  pin 不匹配错误；不通过关闭证书验证制造假成功。
- 在 16 KiB tmpfs 中执行真实软件包下载并要求 ENOSPC，所有故障测试结束后
  运行 `dpkg --audit` 并核对基础包仍处于 `ii` 状态。

验收记录：

- 断网、无效源、TLS 公钥错误和空间不足四项均输出确定的 `REJECTED`，最终
  输出 `RV64-M11 PHASE3 PASS` 并由客户机请求关机。
- 最终复验耗时 228.078 秒，执行 2,367,235,482 machine steps，吞吐
  10.379 Msteps/s；VirtIO Net TX/RX 为 124/217，丢包 0，libslirp 轮询
  错误 0，套接字注册/注销为 14/13。
- 第三阶段完成标记可离线读取；ext4 日志回放后 `e2fsck -fn` 五个检查阶段
  全部通过，无结构或计数错误。
